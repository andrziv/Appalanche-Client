# Appalanche Client Project

This project serves as the frontend of the Appalanche system. This is my first attempt at a Vue application, and is a
tool I created for myself to help me keep track of my job applications and give me a single hub where I could easily
send myself to various job boards. This was designed to connect
to [Appalanche Modulith](https://github.com/andrziv/Appalanche-Modulith).

## Small Tour

The login page (and similarly, the signup page that isn't being shown), used to login to an existing account.
![Dark-theme Login page screenshot](https://github.com/andrziv/Appalanche-Client/blob/master/readme_images/appalanche_login_dark.png "Login page screenshot with empty fields.")

The homepage to allow for quickly browsing your added applications. You can quickly search for applications via the
search bar for position name, company name, or the application ID (if you set one). You can also search using the
other fields with the drop-downs, or you can mix and combine with any of the aforementioned criteria.
![Dark-theme Home page screenshot](https://github.com/andrziv/Appalanche-Client/blob/master/readme_images/appalanche_home_dark.jpg "Home page screenshot with example applications.")

If you click on an application you can open up the editing panel to change any of the attributes you previously set.
You can also add a job description or your notes on the position. You also have access to all the searching
functionality the home page has, if you need to search while in editing mode.
![Dark-theme Editing panel screenshot](https://github.com/andrziv/Appalanche-Client/blob/master/readme_images/appalanche_editing_dark.jpg "Editing panel screenshot on an example 'Junior Robotics Engineer' application at Stark Industries.")

The settings panel is where you can set various quality-of-life links that will be placed on the sub-navigation bar.
Note how in the previous images there were no LinkedIn/GitHub links, but now they've been added via settings.
In addition to personal links, you can add links to job boards that are accessible via the drop-down under the logo.
![Dark-theme Settings screenshot](https://github.com/andrziv/Appalanche-Client/blob/master/readme_images/appalanche_settings_dark.jpg "Settings page screenshot with fields partially filled in and some example job sites.")

Clicking on the "Add Application" button will show the following modal, where you can fill the displayed fields and
Pressing "Save" will create a new application that will quickly show up at the top of your home page listings (assuming
you're on the first page).
![Dark-theme Application creation modal screenshot](https://github.com/andrziv/Appalanche-Client/blob/master/readme_images/appalanche_addapp_dark.png "Screenshot of modal used for submitting a new application with fields unfilled.")

The entire website also has a light-mode in case that's preferrable. Here's an example of the home page but in light
mode.
![Light-theme Home page screenshot](https://github.com/andrziv/Appalanche-Client/blob/master/readme_images/appalanche_home_light.jpg "Light-theme Home page screenshot with example applications.")

## Connecting to Server

### Serving from Modulith

You could modify parts of this application and build it to produce the distributable files needed to serve a website
from a non-Node server (like the Java Modulith that backs this frontend).

Follow these steps to get this to work:

1) Change the configuration files
    1) In `package.json`, replace the `build` run target to the following: `"build": "vite build"`
    2) In `tsconfig.app.json`:
        1) remove the configuration line with the following: `"noEmit": false,`,
        2) and replace it with `"outDir": "./dist/types"`
    3) In `tsconfig.node.json`:
        1) remove the configuration line with the following: `"noEmit": false,`
        2) and replace it with `"outDir": "./dist/types_node"`
    4) In `vite.config.ts`, add `base: '/',` to the top of the config definition (inside `defineConfig({...})`)
2) Run the new build target
    1) in WebStorm you can just click the play button next to it in `package.json`
    2) in other IDEs, just run the following command from a terminal within the project directory: `npm run build`
3) Once the build succeeds, take the files inside the `/dist` directory within the project and paste them within the
   [Appalanche Modulith](https://github.com/andrziv/Appalanche-Modulith)'s `src/main/resources/static` directory.
   In case the `static` directory doesn't exist, just make it (`mkdir static`) under the resources directory.
4) Follow the steps to build the server from that repo's README.

### Containers

There's a `default.conf.template` for an Nginx reverse proxy to serve the same purpose as the Vite proxies when in
production. There's also a `compose.yaml` and `Dockerfile` (almost) ready for deployment on some server. Here are some
steps to follow:

1) Build the image using the Dockerfile
    1) This kind-of depends on what you're building on and where you intend to deploy. The `--push` flags are optional
       (they're for Docker Hub which help with major step #2).
        1) If you're going from ARM to x86, you'll need to use `buildx` (here we're building for a Linux x86 machine,
           ARM would be `linux/arm64`:
            - `docker buildx build --platform linux/amd64 -t ur-user/project:latest --push .`
        2) If you're building from same arch to run on same arch:
            - `docker build -t ur-user/project:latest --push .`
    2) Make sure to keep the `ur-user/project:latest` bit in mind, it's important.
2) Push the image to wherever you want to deploy. Whether it's Docker Hub, or somehow serializing it and sending it via
   other methods (`scp`, email, using a USB, etc...)
    - If you're using Docker Hub, remember to `pull` the images to the server (your tags may vary):
        - `docker pull ur-user/project:latest`
        - There are probably two pulls you need to do: one for the server, one for the client.
3) Create a new compose.yaml file on the server and set it up however you want.
    - As someone who's still kinda fresh to all this stuff, I just like having one big compose file and having all my
      services (fronend, backend, DB) in there connected via a network.
    - The commented-out bits are for your reference.
        - The ${BACKEND_SERVICE_NAME} parts are for an environment variable (this is the mystery sauce for the Nginx
          template config).
    - Update the `image:` part to correctly name your images (you probably didn't create the images with my name)
4) Set up your environment variables (make sure to include `BACKEND_SERVICE_NAME`)
    - The backend service name is just the name of the service. If you're using my compose file for the server,
      then it's just `app`.
5) Set up your SSL certificates under `/certs` wherever your working directory is (or remove the relevant parts from
   the `compose.yaml` and `nginx.conf.tenplate` files if you want to just run the insecure version)
    - Relevant parts to remove/change if you want to just run the system bare:
        - `- "443:443"`in `compose.yaml`
        - ` volumes: - ./certs:/etc/nginx/certs:ro` in `compose.yaml`
        - The first `server` block and everything between and including `ssl_certificate` and `HIGH:!aNULL:!MD5;` in the
          second `server` block in
          `default.conf.template`
        - Replace `listen 443 ssl;` with `listen 80;` in `default.conf.template`
6) Run `docker compose up -d`, and then wait for the server to load. If you set everything up correctly, it should run!

## Future Goals

See the Modulith [project's](https://github.com/andrziv/Appalanche-Modulith) README section of the same name.